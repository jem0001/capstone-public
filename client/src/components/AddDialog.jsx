...
export function AddDialog({
  ...
  const {
    ...
    onSubmit: async (values, actions) => {
      ...
      console.log("selectedsection", values.section);
      const formData = new FormData();
      formData.append("studentId", values.studentId);
      formData.append("firstName", values.firstName);
      formData.append("lastName", values.lastName);
      formData.append("batch", values.batch);
      formData.append("section", values.section.toLowerCase());
        ...
        <form onSubmit={handleSubmit}>
          <CardBody className="flex flex-col gap-4 overflow-visible">
            ...
            <div className="">
              ...
              {values.studentId ? (
                <Button
                  className="w-fit bg-[#03346E] text-white"
                  onClick={handleGenerateQr}>
                  Generate Qr
                </Button>
              ) : (
                <Button className="w-fit bg-[#03346E] text-white" disabled>
                  Generate Qr
                </Button>
              )}
          ...
          <CardFooter className="pt-0">
            <Button type="submit" className="bg-[#059212]" fullWidth>
              Add
            </Button>
            <Typography variant="small" className="mt-2 flex justify-center">
              You may click generate qr button upon filling up student Id.
            </Typography>
          </CardFooter>
        </form>
      </Card>
